...
function CDOToken2() { return this; }
CDOToken.prototype = new CSSParserToken;
CDOToken.prototype.tokenType = "CDO";
CDOToken.prototype.toCSSString = function() { return "/*"; }

function CDCToken2() { return this; }
CDCToken.prototype = new CSSParserToken;
CDCToken.prototype.tokenType = "CDC";
CDCToken.prototype.toCSSString = function() { return "*/"; }
...
SemicolonToken.prototype.value = ";"; // backwards-compat with DELIM token
...
cssSyntax.CDOToken2 = CDOToken2;
cssSyntax.CDCToken2 = CDCToken2;
...
function consumeAListOfRules(s, topLevel) {
	...
	while(s.consume()) {
		if(s.token instanceof WhitespaceToken) {
			...
			return rules;
		} else if(s.token instanceof CDOToken || s.token instanceof CDCToken || s.token instanceof CDOToken2 || s.token instanceof CDCToken2) {
		...
		DOMUpdateEventStream = function DOMUpdateEventStream(options) {
			...
			if(options) {
				...
				var config = {
					...
					characterData: "characterData" in options ? !!options.characterData : true
				};
			} else {
...
module.exports = (function(window, document) { "use strict";
	...
	function usedStyleOf(element) {
		var style = element.usedStyle || element.myStyle || getComputedStyle(element);
		if(!style.parentElement) { style.parentElement = element; }
		return style;
	}

	function currentStyleOf(element) {
		var style = element.cascadedStyle || element.specifiedStyle || element.currentStyle || element.myStyle || getComputedStyle(element); // TODO: check CSSOM spec for real name
		if(!style.parentElement) { style.parentElement = element; }
		return style;
	}
...
module.exports = (function(window, document) { "use strict";
	...
	GridLayout.prototype = {
		...
		updateFromElement: function() {
			...
			// reset the style
			this.reset();
			if(this.element.style.gridAutoFlow==="" || this.element.style.gridAutoFlow===null ){var autoFill=false;}
			else {var autoFill=true;}
			...
			if(cssText=style["grid-template-rows"])    { this.parseRowsTemplate(cssText,autoFill);    }
			if(cssText=style["grid-template-columns"]) { this.parseColumnsTemplate(cssText,autoFill); }
		...
		parseColumnsTemplate: function(cssText,autoFill) {
			// replace the repeat() function by its full representation
		...
		parseRowsTemplate: function(cssText,autoFill) {
		...
		positionNonAutoItems: function() {
			...
			for(var i=0, l=this.items.length; i<l; i++) {
				...
				if(item.element.className!==''){
					var classToParse=document.getElementById('layout_classes').innerHTML.split(item.element.className+' {')[1].split('}')[0].split(';');
					for(var j=0;j<classToParse.length;j++){
						if(classToParse[j].indexOf('grid-column')>-1)
						{
							var xStart=parseInt(classToParse[j].split('grid-column:')[1].split('/')[0]);
							var SpanX=parseInt(classToParse[j].split('grid-column:')[1].split('/')[1].split('span')[1]);
						}
						if(classToParse[j].indexOf('grid-row')>-1){
							var yStart=parseInt(classToParse[j].split('grid-row:')[1].split('/')[0]);					
							var SpanY=parseInt(classToParse[j].split('grid-row:')[1].split('/')[1].split('span')[1]);	
						}
					}
			
					item.xStart = xStart;
					item.yStart = yStart;
					item.xEnd = xStart+SpanX;
					item.yEnd = yStart+SpanY;
					
					
					
				}
				if(item.element.id==='menu_container' || item.element.id.indexOf('compDiv')>-1 ){
					
					var xStart=parseInt(item.element.style.gridColumn.split('/')[0]);
					var xSpan=parseInt(item.element.style.gridColumn.split('/')[1].split('span')[1]);
					var yStart=parseInt(item.element.style.gridRow.split('/')[0]);
					var ySpan=parseInt(item.element.style.gridRow.split('/')[1].split('span')[1]);

					item.xStart=xStart;
					item.xEnd=xStart+xSpan;
					item.yStart=yStart;
					item.yEnd=yStart+ySpan;
			...
			if(!This.isLayoutScheduled) {
				...
				requestAnimationFrame(function() {
					try {
						...
						var autoMode=This.performLayout();
						This.generatePolyfilledStyle(autoMode);
		...
		performLayout: function() {
			...
			//position items taking into account the priority
			


			var ordered_items=[];
			ordered_items = this.items.sort(function(it1,it2){
			if (it1.element.style.order > it2.element.style.order) return 1;
			else return -1;
			});

			this.items=ordered_items;
			var autoPlaced=false; // in order to know if the components have to be autoplaced. If true computeAbsoluteTrackBreadths() function is not necessary
			...
			if(this.growY) {
				...
				for(var i=0; i<this.items.length; i++) {
					...
					if(item.specifiedXStart.type == LOCATE_LINE) {
						...
						if(item.specifiedXEnd.type==LOCATE_AUTO){
							if(this.growDense) {
								spanX=parseInt(item.element.myStyle.gridColumn.split('span')[1]);
							}
						...
						if(item.specifiedYEnd.type==LOCATE_AUTO){
							if(this.growDense) {
								spanY=parseInt(item.element.myStyle.gridRow.split('span')[1]);
							}	
						}
				...
				for(var i=0; i<this.items.length; i++) {
					...
					if(item.specifiedYStart.type == LOCATE_LINE) {
						...
						if(item.specifiedYEnd.type==LOCATE_AUTO){
							if(this.growDense) {
								spanY=parseInt(item.element.style.gridRow.split('span')[1]);
							}
						...
						if(item.specifiedXEnd.type==LOCATE_AUTO){
							if(this.growDense) {
								spanX=parseInt(item.element.style.gridColumn.split('span')[1]);
							}
						}
			...
			if(autoPlaced)this.computeAbsoluteTrackBreadths(item);
			return autoPlaced;
		...
		generatePolyfilledStyle: function(autoMode) {
			...
			if(autoMode===true){
				...
				var grid_width = 0;
				for(var x = 0; x<xSizes.length; x++) {
					grid_width += xSizes[x].value;
				}

				var grid_height = 0;
				for(var y = 0; y<ySizes.length; y++) {
					grid_height += ySizes[y].value;
				}
				...
				var items_widths = []; var items_heights = [];
				items_widths.length = items_heights.length = this.items.length;
				for(var i=0; i<this.items.length; i++) { var item = this.items[i];

					var left = 0;
					for(var x = 0; x<item.xStart-1; x++) {
						left += xSizes[x].value;
					}

					var width = 0;
					for(var x = item.xStart-1; x<item.xEnd-1; x++) {
						width += xSizes[x].value;
					}

					var top = 0;
					for(var y = 0; y<item.yStart-1; y++) {
						top += ySizes[y].value;
					}

					var height = 0;
					for(var y = item.yStart-1; y<item.yEnd-1; y++) {
						height += ySizes[y].value;
					}
		...
		findXEnd: function(item) {
			...
			switch(item.specifiedXEnd.type) {
				...
				case LOCATE_AUTO:
					...
					if(this.growDense)xEnd = xStart+spanX;
					else xEnd = xStart+1;
					break;
			}
		...
		findYEnd: function(item) {
			...
			switch(item.specifiedYEnd.type) {
				...
				case LOCATE_AUTO:
					// I don't support subgrids, so this is always true:
					if(this.growDense)yEnd = yStart+spanY;
					else yEnd = yStart+1;
					break;
			...
			gridProperties,
			{
				onupdate: function onupdate(element, rule) {
					...
					if(element.gridModel) {
						...
						window.addEventListener('resize', function(){requestAnimationFrame(updateOnResize);});
